...
for scene in "${SCENES[@]}"; do
  ...
  if [ ! -d "results/vggsfm/ETH3D/${scene}/colmap/sparse" ]; then
    echo "VGGSfm model not found for ${scene}. Skipping..."
    continue
  fi
  echo "VGGSfm: Processing ${scene}"
  python main.py --est-model-path results/vggsfm/ETH3D/${scene}/colmap/sparse --gt-model-path datasets/ETH3D/${scene}/dslr_calibration_jpg
done
