...
class Parser:
    ...
    def __init__(
        ...
        self.colmap_model = read_model(colmap_dir)

        for image in self.colmap_model.images.values():
            if image.registered:
                self.registered_images.append(image)
                self.registered_images_name.append(image.name)
        ...
        for image_name in self.image_list:
            if image_name not in self.registered_images_name:
                # Find the closest image in the list to the source image.
                closest_image = _find_closest_image(
                    src_image=image_name, image_list=self.registered_images_name
                )
                if closest_image:
