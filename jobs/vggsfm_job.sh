...
process_scene() {
    ...
    log "Running VGG-SfM pipeline on scene: $scene"
        if ! "$PYTHON_BIN" vggsfm/demo.py camera_type=SIMPLE_RADIAL SCENE_DIR="$scene_dir" OUTPUT_DIR="$out_dir" extra_pt_pixel_interval=10 concat_extra_points=True extra_by_neighbor=10 2>&1 | tee -a "$LOG_FILE"; then
